...
class MyDaemon(daemon):
    ...
    def run(self):
            while True:
                ...
                weather_com_result = pywapi.get_weather_from_weather_com(LocationCode, units='imperial')

                # extract current data and last data timestamp, reformat
                ts = weather_com_result['current_conditions']['last_updated']
                data_time = time.strptime(ts, "%m/%d/%y %I:%M %p %Z")
                print(ts)
                today = time.strftime("%d %b %H:%M", data_time)

                test_windSpeed = weather_com_result['current_conditions']['wind']['speed']
                if 'calm' in test_windSpeed:
                    currWind = test_windSpeed
                else:
                    currWind = "{:.0f} mph ".format(int(weather_com_result['current_conditions']['wind']['speed'])) + \
                ...
                currPress = weather_com_result['current_conditions']['barometer']['reading'] + " in Hg"
                uv = "UV {}".format(weather_com_result['current_conditions']['uv']['text'])
                humid = "RH {}%".format(weather_com_result['current_conditions']['humidity'])
                ...
                for i in range(start, 5):
                    ...
                    test_WindSpd[i] = weather_com_result['forecasts'][i]['day']['wind']['speed']
                    if 'calm' in test_WindSpd[i]:
                        forecastWinds[i] = test_WindSpd[i]
                    else:
